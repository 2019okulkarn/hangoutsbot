    ...
    for module_path, plugin in plugins.tracking.list.items():
        ...
        if len(args) == 0 or args[0] in plugin["metadata"]["module"] or args[0] in module_path:
            ...
            if len(plugin["commands"]["admin"]) > 0:
                lines.append(
                    "<b>admin commands:</b> <pre>{}</pre>".format(", ".join(plugin["commands"]["admin"])))
            ...
            if len(user_only_commands) > 0:
                lines.append(
                    "<b>user commands:</b> <pre>{}</pre>".format(", ".join(user_only_commands)))

            """handlers"""
            if len(plugin["handlers"]) > 0:
                lines.append("<b>handlers:</b>")
                lines.append("<br />".join(["... <b><pre>{}</pre></b> (<pre>{}</pre>, p={})".format(
                    f[0].__name__, f[1], str(f[2])) for f in plugin["handlers"]]))

            """shared"""
            if len(plugin["shared"]) > 0:
                lines.append("<b>shared:</b> " + ", ".join(
                    ["<pre>{}</pre>".format(f[1].__name__) for f in plugin["shared"]]))

            """threads"""
            if len(plugin["threads"]) > 0:
                lines.append(
                    "<b>threads:</b> {}".format(len(plugin["threads"])))
            ...
            if len(plugin["commands"]["tagged"]) > 0:
                ...
                for command_name, type_tags in plugin["commands"]["tagged"].items():
                    ...
                    lines.append(
                        "... <b><pre>{}</pre></b>: <pre>{}</pre>".format(command_name, ', '.join(matches)))
