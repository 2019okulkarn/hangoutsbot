...
# for question/bug reports please visit http://www.fuerstjh.com and submit
# through contact page.

# only been tested on python 3.2
# Currently if an error is encountered the API does not handle the
# exception it raises it to the caller.
...
# get scores for
# NOTE: the team names must appear as listed on espn bottomline. To see
# list run once with no filter
...
def get_scores(league, team_filter=None):
    ...
    try:
        ...
        for i in range(1, len(data)):
            ...
            # get rid of junk at beginning of line, remove ^ which marks team
            # with ball
            main_str = data[i][data[i].find('=') + 1:].replace('^', '')

            # extract time, you can use the ( and ) to find time in string
            time = main_str[main_str.rfind(
                '('):main_str.rfind(')') + 1].strip()
