...
def chatMessageEvent2SlackText(event):
    ...
    for segment in event.segments:
        if segment.type_ == hangups.schemas.SegmentType.TEXT:
            ...
            logger.warning(
                'Ignoring unknown chat message segment type: %s',
                segment.type_)
...
class SlackMessage(object):
    ...
    def __init__(self, slackrtm, reply):
        ...
        if reply['type'] in [
            ...
            raise ParseError(
                'not a "message" type reply: type=%s' %
                reply['type'])
...
class SlackRTM(object):
    ...
    def __init__(self, sink_config, bot, loop, threaded=False):
        ...
        if threaded:
            ...
            logger.info(
                'started RTM connection for SlackRTM thread %s',
                pprint.pformat(
                    threading.current_thread()))
            for t in threading.enumerate():
                if t.name == self.threadname and t != threading.current_thread():
                    logger.info(
                        'old thread found: %s - killing it',
                        pprint.pformat(t))
                    t.stop()
    ...
    def get_slackDM(self, userid):
        if userid not in self.dminfos:
            self.dminfos[userid] = self.api_call(
    ...
    def get_channel_users(self, channelid, default=None):
        ...
        if channelid.startswith('C'):
            if channelid not in self.channelinfos:
                self.update_channelinfos()
            if channelid not in self.channelinfos:
                logger.error(
                    'get_channel_users: Failed to find channel %s' %
                    channelid)
                return None
            else:
                channelinfo = self.channelinfos[channelid]
        else:
            if channelid not in self.groupinfos:
    ...
    def handleCommands(self, msg):
        cmdfmt = re.compile(
            r'^<@' +
            self.my_uid +
            r'>:?\s+(help|whereami|whoami|whois|admins|hangoutmembers|hangouts|listsyncs|syncto|disconnect|setsyncjoinmsgs|sethotag|setimageupload|setslacktag|showslackrealnames)',
            re.IGNORECASE)
            ...
            for sync in self.get_syncs(channelid=msg.channel):
                ...
                for c in self.bot.list_conversations():
                    if c.id_ == sync.hangoutid:
                        ...
                        break
                message += '%s aka %s (%s):\n' % (hangoutname,
                                                  sync.hotag if sync.hotag else 'untagged',
                                                  sync.hangoutid)
                for u in conv.users:
    ...
    def setimageupload(self, channel, hangoutid, upload):
        ...
        logger.info(
            'setting image_upload=%s for sync=%s',
            upload,
            sync.toDict())
    ...
    def showslackrealnames(self, channel, hangoutid, realnames):
        ...
        logger.info(
            'setting showslackrealnames=%s for sync=%s',
            realnames,
            sync.toDict())
    ...
    def handle_reply(self, reply):
        ...
        for sync in self.get_syncs(channelid=msg.channel):
            ...
            if msg.from_ho_id != sync.hangoutid:
                ...
                logger.debug(
                    'forwarding to HO %s: %s',
                    sync.hangoutid,
                    response.encode('utf-8'))
                if msg.file_attachment:
                    if sync.image_upload:
                        self.loop.call_soon_threadsafe(
                            asyncio.async, self.upload_image(
                                sync.hangoutid, msg.file_attachment))
                    else:
...
#                print('slackrtm: NO image in message: "%s"' % event.text)
    ...
    def handle_ho_membership(self, event):
        ...
        for user_id in event.conv_event.participant_ids:
            ...
            links.append(
                u'<https://plus.google.com/%s/about|%s>' %
...
def _initialise(bot):
    ...
    plugins.register_admin_command(["slack_help",
                                    "slacks",
                                    "slack_channels",
                                    "slack_listsyncs",
                                    "slack_syncto",
...
usage: /bot slack_channels <teamname>"""
    ...
    if not slackrtm:
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'ERROR: Could not find a configured slack team with name "%s", use /bot slacks to list all teams' %
                    slackname, is_bold=True)])
        return
    ...
    for cid in slackrtm.channelinfos:
        if not slackrtm.channelinfos[cid]['is_archived']:
            segments.append(
                hangups.ChatMessageSegment(
                    '%s (%s)' %
                    (slackrtm.channelinfos[cid]['name'], cid)))
            segments.append(
                hangups.ChatMessageSegment(
                    '\n', hangups.SegmentType.LINE_BREAK))
    ...
    for gid in slackrtm.groupinfos:
        if not slackrtm.groupinfos[gid]['is_archived']:
            segments.append(
                hangups.ChatMessageSegment(
                    '%s (%s)' %
                    (slackrtm.groupinfos[gid]['name'], gid)))
            segments.append(
                hangups.ChatMessageSegment(
                    '\n', hangups.SegmentType.LINE_BREAK))
...
def slack_users(bot, event, *args):
    ...
    if not channelname:
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'ERROR: Could not find a channel with id "%s" in team "%s", use /bot slack_channels %s to list all teams' %
                    (channelid, slackname, slackname), is_bold=True)])
        return
    ...
    segments.append(
        hangups.ChatMessageSegment(
            'Slack users in channel %s:' %
            (channelname), is_bold=True))
    segments.append(hangups.ChatMessageSegment(
        '\n', hangups.SegmentType.LINE_BREAK))
...
usage: /bot slack_listsyncs"""
    segments = [
        hangups.ChatMessageSegment(
            'list of currently synced conversations:',
            is_bold=True),
        hangups.ChatMessageSegment(
            '\n',
            hangups.SegmentType.LINE_BREAK)]
...
usage: /bot slack_syncto <teamname> <channelid>"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'Already syncing this Hangout to %s:%s.' %
                    (slackname, channelname), is_bold=True)])
    else:
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, I will now sync all messages in this Hangout to %s:%s.' %
                    (slackname, channelname), is_bold=True)])
...
usage: /bot slack_disconnect <teamname> <channelid>"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'This Hangout is NOT synced to %s:%s.' %
                    (slackname, channelname), is_bold=True)])
    else:
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, I will no longer sync messages in this Hangout to %s:%s.' %
                    (slackname, channelname), is_bold=True)])
...
usage: /bot slack_setsyncjoinmsgs <teamname> <channelid> {true|false}"""
    ...
    if enable.lower() in ['true', 'on', 'y', 'yes']:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'sorry, but "%s" is not "true" or "false"' %
                    enable, is_bold=True)])
        return
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, I will %s sync join/leave messages in this Hangout with %s:%s.' %
                    (('now' if enable else 'no longer'), slackname, channelname), is_bold=True)])
...
usage: /bot slack_setimageupload <teamname> <channelid> {true|false}"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, I will %s upload images to this Hangout when shared in %s:%s.' %
...
usage: /bot slack_hotag <teamname> <channelid> {<tag>|none}"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, messages from this Hangout will %s in slack channel  %s:%s.' %
                    (oktext, slackname, channelname), is_bold=True)])
...
usage: /bot slack_slacktag <teamname> <channelid> {<tag>|none}"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, messages from slack channel %s:%s will %s in this Hangout.' %
                    (oktext, slackname, channelname), is_bold=True)])
...
usage: /bot slack_showslackrealnames <teamname> <channelid> {true|false}"""
    ...
    try:
        ...
        bot.send_message_segments(
            event.conv, [
                hangups.ChatMessageSegment(
                    'OK, I will display %s in this Hangout when syncing from %s:%s.' %
