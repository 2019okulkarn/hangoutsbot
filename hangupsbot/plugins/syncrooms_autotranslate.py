    ...
    if context and "autotranslate" in context:
        ...
        for send in broadcast_list:
            ...
            if origin_language != target_language:
                logger.debug("translating {} to {}".format(
                    origin_language, target_language))
                translated = gs.translate(
                    _autotranslate["event_text"], target_language)
...
def _get_room_language(bot, conversation_id, default="en"):
    syncroom_language = bot.conversation_memory_get(
        conversation_id, 'syncroom_language')
    ...
    for iso_language in language_map:
        ...
        if language.lower() in text_language.lower() or language == iso_language.upper():
            bot.conversation_memory_set(
                event.conv_id, 'syncroom_language', iso_language)
            yield from bot.coro_send_message(
                event.conv,
