...
def check_quote_admin(bot, event):
    tags = bot.user_memory_get(event.user.id_.chat_id, 'tags')
    if tags:
        if 'quote-admin' in tags:
            return True
        else:
            return False
    else:
...
def retrieve(conn, id_, author, full=True):
    ...
    if not author:
        ...
        if not full:
            c.execute(
                'SELECT * FROM quotes WHERE author = ? ORDER BY RANDOM() LIMIT 1', [id_])
            q = c.fetchone()
            msg = format_quote(q)
        else:
...
def quote(bot, event, *args):
    ...
    try:
        ...
        if args[0] not in ['-a', '-d', '-l', '-e'] and args[0].startswith('-'):
            ...
            if is_admin(bot, event) or check_quote_admin(bot, event):  # admin only quote functions
                if args[0] == "-a":
