...
class TelegramBot(telepot.async.Bot):
    ...
    def on_user_join(bot, chat_id, msg):
        print(
            "New User: {name}".format(
                name=msg['left_chat_participant']['first_name']))

    @staticmethod
    def on_user_leave(bot, chat_id, msg):
        print("{name} Left the gorup".format(
            name=msg['left_chat_participant']['first_name']))

    @staticmethod
    def on_location_share(bot, chat_id, msg):
        print("{name} shared a location".format(
            name=msg['left_chat_participant']['first_name']))
    ...
    def handle(self, msg):
        ...
        if flavor == "normal":  # normal message
            ...
            query_id, from_id, query_string = telepot.glance2(
                msg, flavor=flavor)
            print("inline_query")

        elif flavor == "chosen_inline_result":
            result_id, from_id, query_string = telepot.glance2(
                msg, flavor=flavor)
...
def tg_util_create_gmaps_url(lat, long, https=True):
    return "{https}://maps.google.com/maps?q={lat},{long}".format(
        https='https' if https else 'http', lat=lat, long=long)
...
def tg_on_message(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        text = "<b>{uname}</b> <b>({gname})</b>: {text}".format(
        ...
        logger.info(
            "[TELESYNC] Telegram message forwarded: {msg} to: {ho_conv_id}".format(
                msg=msg['text'],
...
def tg_on_photo(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        ...
        # get photo id so we can download it
        ...
        text = "Uploading photo from <b>{uname}</b> in <b>{gname}</b>...".format(
            uname=msg['from']['first_name'], gname=tg_util_get_group_name(msg))
        yield from tg_bot.ho_bot.coro_send_message(ho_conv_id, text)
...
def tg_on_user_join(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        text = "<b>{uname}</b> joined <b>{gname}</b>".format(
            uname=msg['new_chat_participant']['first_name'],
            gname=tg_util_get_group_name(msg))
...
def tg_on_user_leave(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        text = "<b>{uname}</b> left <b>{gname}</b>".format(
            uname=msg['left_chat_participant']['first_name'],
            gname=tg_util_get_group_name(msg))
...
def tg_on_location_share(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        ...
        logger.info(
            "[TELESYNC] Telegram location forwarded: {msg} to: {ho_conv_id}".format(
                msg=maps_url, ho_conv_id=ho_conv_id))
...
def _initialise(bot):
    ...
    if not bot.config.exists(['telegram_bot_api_key']):
        bot.config.set_by_path(
            ...
            "PUT_YOUR_TELEGRAM_API_KEY_HERE")
...
def telesync(bot, event, *args):
    ...
    if len(parameters) > 1:
        ...
        raise RuntimeError(
            "plugins/telesync: it seems something really went wrong, you should not see this error")
...
def is_animated_photo(file_name):
    return True if get_photo_extension(file_name).endswith(
        (".gif", ".gifv", ".webm", ".mp4")) else False
...
def _on_hangouts_message(bot, event, command=""):
    ...
    if event.conv_id in ho2tg_dict:
        user_gplus = 'https://plus.google.com/u/0/{uid}/about'.format(
            uid=event.user_id.chat_id)
        text = "[{uname}]({user_gplus}) *({gname})*: {text}".format(
            uname=event.user.full_name,
        ...
        if has_photo:
            ...
            if tg_bot.ho_bot.config.get_by_path(
                    ['telesync_do_not_keep_photos']):
                os.remove(photo_path)  # don't use unnecessary space on disk


def create_membership_change_message(
        user_name,
        user_gplus,
        group_name,
        membership_event="left"):
    text = "[{uname}]({user_gplus}) {membership_event} *({gname})*".format(
        uname=user_name,
        user_gplus=user_gplus,
        gname=group_name,
...
def _on_membership_change(bot, event, command=""):
    ...
    text = create_membership_change_message(
        names, user_gplus, event.conv.name, membership_event)
