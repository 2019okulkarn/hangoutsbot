...
def lookup(bot, event, *args):
    ...
    spreadsheet_url = bot.get_config_suboption(
        event.conv_id, 'spreadsheet_url')
    # Name of table class to search. Note that 'waffle' seems to be the
    # default for all spreadsheets
    table_class = "waffle"
    ...
    logger.debug("{0} ({1}) has requested to lookup '{2}'".format(
        event.user.full_name, event.user.id_.chat_id, keyword))
    ...
    if counter > counter_max:
        htmlmessage += _('<br />{0} rows found. Only returning first {1}.').format(
            counter, counter_max)
        if counter_max == 5:
            htmlmessage += _('<br />Hint: Use <b>/bot lookup <{0} {1}</b> to view {0} rows').format(
                counter_max * 2, keyword)
